...
async function createGameElement(game) {
    ...
    if (juegosMostrados.has(game.id)) {
        ...
        if (platformPriority[shownPlatform] < platformPriority['Móvil']) {
            console.log('Juego ya mostrado en plataforma con mayor prioridad:', game.name);
            return null;
        }
    }

    // Verificar si el juego está disponible en móviles
    const isMobile = game.platforms.some(platform => 
        groupedPlatforms['Móvil'].some(p => p.name === platform.platform.name)
    );
    
    if (!isMobile) {
        console.log('Juego no disponible en móviles:', game.name);
        return null;
    }
    ...
    window.lastShownPlatform[game.id] = 'Móvil';
...
async function loadGames(genreId, page = 1) {
    ...
    const platformIds = groupedPlatforms['Móvil'].map(p => p.id);
    ...
    const categorySectionId = `movil-${genre.slug}`;
...
async function main() {
    console.log('Iniciando carga de juegos móviles...');
    const genres = await getGenres();
