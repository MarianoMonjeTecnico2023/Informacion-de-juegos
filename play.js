...
async function createGameElement(game) {
    ...
    if (juegosMostrados.has(game.id)) {
        ...
        if (platformPriority[shownPlatform] < platformPriority['PlayStation']) {
            console.log('Juego ya mostrado en plataforma con mayor prioridad:', game.name);
            return null;
        }
    }

    // Verificar si el juego está disponible en PlayStation
    const isPlayStation = game.platforms.some(platform => 
        groupedPlatforms['PlayStation'].some(p => p.name === platform.platform.name)
    );
    
    if (!isPlayStation) {
        console.log('Juego no disponible en PlayStation:', game.name);
        return null;
    }
    ...
    window.lastShownPlatform[game.id] = 'PlayStation';
...
async function loadGames(genreId, page = 1) {
    ...
    const platformIds = groupedPlatforms['PlayStation'].map(p => p.id);
    ...
    const categorySectionId = `play-${genre.slug}`;
...
async function main() {
    console.log('Iniciando carga de juegos de PlayStation...');
    const genres = await getGenres();
