...
async function createGameElement(game) {
    ...
    if (juegosMostrados.has(game.id)) {
        ...
        if (platformPriority[shownPlatform] < platformPriority['Otras']) {
            console.log('Juego ya mostrado en plataforma con mayor prioridad:', game.name);
            return null;
        }
    }

    // Verificar si el juego está disponible en otras plataformas
    const isOther = game.platforms.some(platform => 
        groupedPlatforms['Otras'].some(p => p.name === platform.platform.name)
    );
    
    if (!isOther) {
        console.log('Juego no disponible en otras plataformas:', game.name);
        return null;
    }
    ...
    window.lastShownPlatform[game.id] = 'Otras';
...
async function loadGames(genreId, page = 1) {
    ...
    const platformIds = groupedPlatforms['Otras'].map(p => p.id);
    ...
    const categorySectionId = `otras-${genre.slug}`;
...
async function main() {
    console.log('Iniciando carga de juegos de otras plataformas...');
    const genres = await getGenres();
