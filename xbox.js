...
async function createGameElement(game) {
    ...
    if (juegosMostrados.has(game.id)) {
        ...
        if (platformPriority[shownPlatform] < platformPriority['Xbox']) {
            console.log('Juego ya mostrado en plataforma con mayor prioridad:', game.name);
            return null;
        }
    }

    // Verificar si el juego está disponible en Xbox
    const isXbox = game.platforms.some(platform => 
        groupedPlatforms['Xbox'].some(p => p.name === platform.platform.name)
    );
    
    if (!isXbox) {
        console.log('Juego no disponible en Xbox:', game.name);
        return null;
    }
    ...
    window.lastShownPlatform[game.id] = 'Xbox';
...
async function loadGames(genreId, page = 1) {
    ...
    const platformIds = groupedPlatforms['Xbox'].map(p => p.id);
    ...
    const categorySectionId = `xbox-${genre.slug}`;
...
async function main() {
    console.log('Iniciando carga de juegos de Xbox...');
    const genres = await getGenres();
